...
const QUICK_FILTERS = [
  ...
  { label: "Adventure", icon: MapIcon, color: "#3b82f6", gradient: "from-blue-500/20 to-sky-500/10", sort: "downloads" as SortIndex, category: "adventure" },
    ...
    <div className="home-slide-up" style={{ animationDelay: `${delay}s` }}>
      <div className="flex items-center justify-between mb-3 px-1">
        <div>
          ...
          <p className="mt-1 pl-6 text-[11px] font-medium uppercase tracking-[0.16em] text-(--color-text-secondary)">
            {mods.length} picks
          </p>
        </div>
    ...
    >
      ...
      <div className="relative z-10 flex h-full flex-col">
        ...
        <div className="mt-4 min-h-[3.6rem]">
          <h3 className="line-clamp-2 pt-1 text-base leading-tight font-black text-(--color-text-primary) group-hover:text-(--color-accent) transition-colors tracking-tight [overflow-wrap:anywhere]">
            {mod.title}
          </h3>
          <p className="truncate pt-1 text-xs text-(--color-text-secondary) font-semibold">
            {mod.author}
          </p>
        </div>
        ...
        <div className="mt-auto border-t border-white/6 pt-3">
          <button
            ...
            disabled={installing}
            className="w-full flex items-center justify-center gap-2 rounded-xl bg-(--color-accent) px-3 py-2 text-[11px] font-bold text-white transition-all cursor-pointer hover:opacity-95 disabled:opacity-60 disabled:cursor-default"
          >
            <Download size={12} />
      ...
      <div className="relative z-10 flex h-full flex-col">
        <div className="flex items-start gap-5">
          ...
          <div className="min-w-0 flex-1 pt-1">
            <h3 className="line-clamp-2 text-base leading-tight font-black text-(--color-text-primary) group-hover:text-(--color-accent) transition-colors tracking-tight [overflow-wrap:anywhere]">
              {mod.title}
            </h3>
            <p className="truncate pt-1 text-xs text-(--color-text-secondary) font-semibold">
              {mod.author}
        ...
        <div className="mt-auto pt-5">
          <button
            ...
            disabled={installing}
            className="flex items-center justify-center gap-2 rounded-xl bg-(--color-accent) px-4 py-2 text-[11px] font-bold text-white transition-all cursor-pointer hover:opacity-95 disabled:opacity-60 disabled:cursor-default"
          >
            <Download size={12} />
            {installing ? "Installing..." : "Install"}
          </button>
  ...
  return (
    <div className="h-full overflow-y-auto">
      ...
      <div className="relative px-8 pt-8 pb-4 overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="home-slide-up" style={{ animationDelay: "0s" }}>
            <h1 className="text-[2.5rem] font-black tracking-tight text-(--color-text-primary)">
          ...
          {preferredInstance && (
            <div
              className="mt-4 inline-flex items-center gap-2 rounded-full border border-(--color-accent)/20 bg-(--color-accent)/8 px-3 py-1.5 text-[11px] font-semibold text-(--color-accent) home-slide-up"
          ...
          <form onSubmit={handleSearch} className="flex gap-2 mt-6 home-slide-up" style={{ animationDelay: "0.1s" }}>
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-3 rounded-[1.125rem] bg-(--color-surface-secondary) border border-(--color-border) text-sm text-(--color-text-primary) placeholder:text-(--color-text-secondary)/60 focus:outline-none focus:ring-2 focus:ring-(--color-accent)/40 focus:border-transparent transition-all"
              />
            </div>
            <button type="submit" className="px-6 py-3 rounded-[1.125rem] bg-(--color-accent) hover:bg-(--color-accent-hover) text-white text-sm font-bold transition-all cursor-pointer shadow-sm shadow-(--color-accent)/12 hover:shadow-md hover:-translate-y-0.5 active:translate-y-0">
              Search
            </button>
            <button
              type="button"
              onClick={() => setShowFilters(!showFilters)}
              className={`w-11 h-11 rounded-[1.125rem] flex items-center justify-center transition-all cursor-pointer ${
              ...
              {QUICK_FILTERS.map((filter, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`browse-filter-card group relative flex flex-col items-center gap-2 px-3 py-4 rounded-[1.125rem] border transition-all duration-300 cursor-pointer overflow-hidden ${
                      isActive
                        ? "border-transparent shadow-md scale-[1.01]"
                        : "border-(--color-border) bg-(--color-surface-secondary) hover:border-white/10 hover:shadow-sm hover:-translate-y-0.5"
                    }`}
                    style={{
      ...
      <div className="px-8 pb-8">
        {/* Filters panel */}
        {showFilters && (
          <div className="rounded-[1.25rem] bg-(--color-surface-secondary) border border-(--color-border) p-5 space-y-3 mb-5 picker-dropdown">
            ...
            {mods.hasActiveFilters && (
              <div className="flex flex-wrap gap-1.5 pt-1">
                {mods.selectedCategories.map((cat) => (
                  <button key={cat} onClick={() => mods.toggleCategory(cat)} className="flex items-center gap-1 px-2.5 py-1 rounded-md bg-(--color-accent)/10 text-(--color-accent) text-[11px] font-semibold cursor-pointer hover:bg-(--color-accent)/20 transition-colors">
                    {cat} <X size={10} />
                  </button>
                ))}
                {mods.selectedLoaders.map((loader) => (
                  <button key={loader} onClick={() => mods.toggleLoader(loader)} className="flex items-center gap-1 px-2.5 py-1 rounded-md bg-emerald-500/10 text-emerald-500 text-[11px] font-semibold cursor-pointer hover:bg-emerald-500/20 transition-colors">
                    {loader} <X size={10} />
                  </button>
                ))}
                {mods.selectedGameVersions.map((ver) => (
                  <button key={ver} onClick={() => mods.toggleGameVersion(ver)} className="flex items-center gap-1 px-2.5 py-1 rounded-md bg-sky-500/10 text-sky-500 text-[11px] font-semibold cursor-pointer hover:bg-sky-500/20 transition-colors">
                    {ver} <X size={10} />
                  </button>
                ))}
              </div>
            ...
            <div className="rounded-[1.25rem] border border-(--color-border) bg-(--color-surface-secondary) px-5 py-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-2 text-xl font-black tracking-tight text-(--color-text-primary)">
                ...
                {(mods.searchQuery || activeQuickFilter || mods.hasActiveFilters) && (
                  <button
                    ...
                    className="rounded-lg border border-(--color-border) bg-(--color-surface) px-3 py-2 text-xs font-semibold text-(--color-text-primary) transition-colors hover:bg-(--color-surface-tertiary) cursor-pointer"
                  >
                    Reset View
                  </button>
                )}
              ...
              <>
                ...
                {mods.totalPages > 1 && (
                  <div className="flex items-center justify-center gap-1.5 pt-4">
                    <button onClick={() => mods.goToPage(mods.currentPage - 1)} disabled={mods.currentPage === 0} className="w-9 h-9 rounded-lg bg-(--color-surface-secondary) border border-(--color-border) flex items-center justify-center text-(--color-text-secondary) hover:text-(--color-text-primary) transition-colors cursor-pointer disabled:opacity-30 disabled:cursor-default">
                      <ChevronLeft size={16} />
                    </button>
                    {pageNumbers().map((p, i) =>
                      p === "..." ? (
                        ...
                        <button key={p} onClick={() => mods.goToPage(p)} className={`w-9 h-9 rounded-lg text-xs font-bold transition-all cursor-pointer ${mods.currentPage === p ? "bg-(--color-accent) text-white shadow-sm" : "bg-(--color-surface-secondary) border border-(--color-border) text-(--color-text-secondary) hover:text-(--color-text-primary)"}`}>
                          {p + 1}
                        </button>
                      )
                    )}
                    <button onClick={() => mods.goToPage(mods.currentPage + 1)} disabled={mods.currentPage >= mods.totalPages - 1} className="w-9 h-9 rounded-lg bg-(--color-surface-secondary) border border-(--color-border) flex items-center justify-center text-(--color-text-secondary) hover:text-(--color-text-primary) transition-colors cursor-pointer disabled:opacity-30 disabled:cursor-default">
