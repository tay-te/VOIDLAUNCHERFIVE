...
| `launch` | Classpath, LWJGL 2 natives extraction, JVM args (`-Xmx`, `-Djava.library.path`, `-Dvoid.port`, `-Dvoid.token`, `-Xdock:name` on macOS), the mod JAR into `mods/`, spawn, stdout/stderr streamed line by line, exit code. The argument list is cached under `cache/args/<hash>.json`, keyed by profile hash. |
...
- [x] Launch: classpath order, `-Xdock:name` only on macOS and no `-XstartOnFirstThread` anywhere, `-Dvoid.port`/`-Dvoid.token`, full placeholder substitution, argument-cache hit/miss, natives unzipped without `META-INF`, mod JAR installed, stdout+stderr streamed, exit code — spawned against a stand-in for `java`.
